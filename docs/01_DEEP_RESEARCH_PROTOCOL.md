...
Original repo path: `docs/01_DEEP_RESEARCH_PROTOCOL.md`
...
Deep Research Prompt、Algorithm Blueprint 或 M0-B 审查结论都不得直接作为 Codex 可执行任务。若后续需要 Codex 执行，必须在 Owner 确认 Project Brief v0.2 或风险豁免后，由 Architect / Reviewer 转化为 `docs/10_CODEX_NEXT_TASK.md`。
...
10. 在未满足 OAR-M 阶段门控前生成或更新 `docs/10_CODEX_NEXT_TASK.md`。

---

## 14. 与 Codex 任务文件的边界

Deep Research 与 Algorithm Blueprint 的输出是研究资料，不是 Codex 执行任务。

后续若需要 Codex 参与工程实现，必须遵守以下规则：

1. Algorithm Blueprint 必须先经过 M0-B：Algorithm Blueprint Intake & Gap Review；
2. Owner 必须确认 Project Brief v0.2 或明确接受 M0-X 风险豁免；
3. Architect / Reviewer 才能进入 M1 并生成 `docs/10_CODEX_NEXT_TASK.md`；
4. Codex 只能执行 `docs/10_CODEX_NEXT_TASK.md` 中的当前有效任务；
5. 如果只是当前 PR 的补丁修复，应将新的 `CURRENT ACTIVE TASK` 任务块粘贴到 `docs/10_CODEX_NEXT_TASK.md` 顶端，而不是只在对话或 PR 评论中给出修复说明；
6. 任何第三方依赖、源码复制、核心接口或验收标准变更，仍必须经过 Owner 明确确认。
