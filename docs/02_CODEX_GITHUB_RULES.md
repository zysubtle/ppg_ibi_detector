...
Original repo path: `docs/02_CODEX_GITHUB_RULES.md`
...
8. Codex 根据聊天中的临时说明执行，而不是根据 `docs/10_CODEX_NEXT_TASK.md` 中的当前有效任务执行。
...
4. 当前里程碑任务文件，且优先以 `docs/10_CODEX_NEXT_TASK.md` 为准；
...
如果 Owner 或 Architect / Reviewer 在对话中提出了新任务，但没有更新 `docs/10_CODEX_NEXT_TASK.md`，Codex 必须要求先更新任务文件，不得自行执行。
...
2. **顶端追加 / Prepend**：用于当前 PR 或当前 Milestone 的小修任务。新任务块必须放在文件顶端，并标注 `CURRENT ACTIVE TASK`。

如果采用顶端追加方式，Codex 应以最上方的 `CURRENT ACTIVE TASK` 为当前任务；旧任务仅作为背景，不能覆盖当前任务。
...
如果当前任务是 PR 审查后的修复任务，Codex 必须确认 `docs/10_CODEX_NEXT_TASK.md` 顶端存在 `CURRENT ACTIVE TASK`，并按该任务执行。

除非任务文件明确要求新开 PR，否则 PR 修复任务应在当前 PR 分支追加 commit。

### 4.5 PR 描述中的报告要求

Codex 在 PR 描述中必须说明：

1. 本次执行所依据的 `docs/10_CODEX_NEXT_TASK.md` 版本或顶端任务标题；
2. 是否采用覆盖任务还是顶端追加任务；
3. 是否有旧任务仍作为背景；
4. 实际修改范围是否与任务文件一致。
...
24. 伪造 PR 链接；
25. 声称完成但未实际执行；
26. 在未更新 `docs/10_CODEX_NEXT_TASK.md` 的情况下，仅凭聊天或 PR 评论执行新任务。
...
Codex 只能执行 `docs/10_CODEX_NEXT_TASK.md` 当前有效任务授权的内容。

如果执行中发现任务文件不完整、目标冲突、接口不清、测试不可运行、依赖缺失，或对话/PR 评论与 `docs/10_CODEX_NEXT_TASK.md` 不一致，Codex 应停止扩大实现范围，并在报告中说明 Blocking Issues。
